...
<div class="max-w-xl mx-auto bg-[#F4F4F4] p-8 rounded-lg shadow-lg mt-10">
    <h1 class="text-3xl font-semibold mb-6 text-[#2C2C2C]">Editar Proveedor</h1>
    ...
    <form action="{{ route('config.proveedores.update', $proveedor->_id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="rfc" id="rfc" value="{{ old('rfc', $proveedor->rfc) }}" required
                class="w-full px-4 py-2 rounded-md border border-[#B497BD] focus:outline-none focus:ring-2 focus:ring-[#B497BD] focus:border-[#B497BD] bg-white text-[#2C2C2C] placeholder-[#9CA3AF]" />
        </div>

        <div>
            <label for="contacto" class="block mb-2 font-medium text-[#2C2C2C]">Contacto</label>
            <input type="text" name="contacto" id="contacto" value="{{ old('contacto', $proveedor->contacto) }}"
                class="w-full px-4 py-2 rounded-md border border-[#B497BD] focus:outline-none focus:ring-2 focus:ring-[#B497BD] focus:border-[#B497BD] bg-white text-[#2C2C2C] placeholder-[#9CA3AF]" />
        </div>
