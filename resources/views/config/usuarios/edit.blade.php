...
@section('content')
<h2 class="text-3xl font-semibold text-[#2C2C2C] mb-6">👥 Editar Usuario</h2>

<section class="bg-[#F4F4F4] border border-[#B497BD] rounded-xl p-6 shadow-sm max-w-lg mx-auto">

  <form action="{{ route('config.usuarios.update', $usuario->_id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
    @method('PUT')

    <div>
      <label for="name_user" class="block mb-2 font-semibold text-[#2C2C2C]">Nombre</label>
      <input type="text" name="name_user" id="name_user" value="{{ old('name_user', $usuario->name_user) }}"
    ...
    <div>
      ...
      <input type="email" name="correo_user" id="correo_user" value="{{ old('correo_user', $usuario->correo_user) }}"
        class="w-full px-4 py-2 rounded-lg border border-[#B497BD] bg-white text-[#2C2C2C]" required>
      @error('correo_user')
      <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
      @enderror
    ...
    <div>
      ...
      <select name="rol_user" id="rol_user" required
        ...
        <option value="Admin" {{ old('rol_user', $usuario->rol_user) == 'Admin' ? 'selected' : '' }}>Admin</option>
        <option value="Auditor" {{ old('rol_user', $usuario->rol_user) == 'Auditor' ? 'selected' : '' }}>Auditor</option>
        <option value="Empleado" {{ old('rol_user', $usuario->rol_user) == 'Empleado' ? 'selected' : '' }}>Empleado</option>
      </select>
      @error('rol_user')
      <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
      @enderror
    ...
    <div>
      ...
      <select name="estado_user" id="estado_user"
        class="w-full px-4 py-2 rounded-lg border border-[#B497BD] bg-white text-[#2C2C2C]" required>
        <option value="1" {{ old('estado_user', $usuario->estado_user) == '1' ? 'selected' : '' }}>Activo</option>
        <option value="0" {{ old('estado_user', $usuario->estado_user) == '0' ? 'selected' : '' }}>Inactivo</option>
      </select>
      @error('estado_user')
      <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
      @enderror
    ...
    <div>
      ...
      <img src="{{ asset('storage' . $usuario->imagen_user) }}" alt="Imagen Usuario" class="w-20 h-20 rounded-full object-cover mt-1">
      @endif
      @error('imagen_user')
      <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
      @enderror
    ...
    <div>
      <label for="password_user" class="block mb-2 font-semibold text-[#2C2C2C]">Contraseña (dejar vacío para no cambiar)</label>
    ...
    <div class="flex justify-between items-center">
      ...
      <button type="submit" class="px-6 py-2 bg-[#B497BD] text-white rounded-lg hover:bg-[#9B82B1] transition">Actualizar Usuario</button>
    </div>
  </form>
</section>
@endsection
